...
extension UIViewController {
    ...
    func showToastGray(message: String) {
        ...
        toastLabel.font = .spoqaMedium(size: 14)
    ...
    func showToastPurple(message: String) {
        let width_variable: CGFloat = 30
        let toastLabel = UILabel(frame: CGRect(x: width_variable, y: UIScreen.main.bounds.height / 2, width: view.frame.size.width-2*width_variable, height: 44))
        // 뷰가 위치할 위치를 지정해준다. 여기서는 아래로부터 100만큼 떨어져있고, 너비는 양쪽에 10만큼 여백을 가지며, 높이는 35로
